...
- ⚡️ **32 FPS @ 1080p** - 在Intel RealSense D435i上实现流畅检测（超电影级24 FPS标准）
- 🛠️ **三大创新** - LRSA注意力/AKConv/图像增强流水线
- 🚄 **硬件适配** - 通过流水线并行（CPU0:图像增强 → CPU1:YOLO检测）保障帧率稳定
- 📱 **轻量化** - 模型仅需8.8MB内存
...
- 该方法结合UNet网络、伪正态特征生成器、颜色偏移估计（COSE）模块以及颜色调制（COMO）模块，有效解决了复杂环境下的视觉问题。
##### b. NeRCo方案
然而，由于参数量巨大，模型复杂度较高，针对CODE和COMO方案难以对图像进行实时处理的问题，我们采用了ICCV'23提出的NeRCo方法，通过神经表示学习实现无监督低光增强：
- 神经表示归一化（NRN）：MLP网络自动统一输入图像的亮度分布，抑制噪声
- 多模态监督：CLIP模型提供语义指导（如"明亮的信号灯"），避免颜色失真
- 双闭环训练：增强-退化循环保证无配对数据下的稳定性
该方案有效平衡了计算成本和训练难度问题，图像处理速度平均为0.2s,基本实现了实时低光增强，有效提升了图像细节和颜色的表现。
...
- 这一方法通过增强红/绿色通道，能够减少由于环境光照变化、物体遮挡或信号标志颜色偏差带来的误判问题，从而确保识别系统的高准确性和可靠性。
...
#### (2) 性能提升
- 通过将LRSA与YOLOV8 C2F模块结合，我们强化了模型在小目标和复杂区域中的特征捕捉能力。LRSA的加入显著提升了YOLOv11模型在不同尺度目标的适应性，提高了模型对复杂场景的理解和抗干扰性。
- 在实验中，LRSA加入p3位置时，相较于加入p4位置，mAP50提升了0.03，最终达到了FLOPS = 8.8和mAP50 = 0.652的最优性能。

### 3. 模型部署与实时检测
我们将改进后的YOLO模型部署在Realsense D435i摄像头上，通过摄像头捕获视频流，实现了实时检测。借助Realsense的深度感知能力和高效的嵌入式处理技术，我们的模型能够在复杂的轨道交通环境中实时识别信号标志，并做出快速反应。

## 🚀 实时性能保障
| 指标               | 值             | 行业标准对比 |  
|--------------------|----------------|-------------|
| 帧率(FPS)         | **32**         | >24 FPS (电影级) |  
| 单帧延迟          | 31ms           | <100ms (轨道交通要求) |  
| 适用车速          | ≤120km/h       | 市区列车典型时速 |  

## ⭐模型性能

### 模型优化实验 (FLOPS vs. mAP50)
...
根据要求，我们在根目录的datasets文件夹中完成了训练集，测试集，和验证集的划分。（由于数据集庞大，没有上传至此仓库，开发者可直接从官网下载）
